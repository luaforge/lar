    ...
    if (pos) then
      ...
      if Private.openedFiles[filename] then
        ...
        if gfile then
          ...
          if tarfile then
            Private.openedFiles[filename] = tarfile
            return tarfile:open(remain)
          end
        else
          current = current.."/"
        end
...
Private.require = function (packagename)
  ...
  local comppath = string.gsub(path, "?", packagename)
  for p in string.gfind(comppath, "([^;]+)") do
    local chunk, err = loadfile(p)
    if chunk then
      _REQUIREDNAME = packagename
      res = chunk() or true
      _LOADED[packagename] = res
    end
  end
  return res
end
